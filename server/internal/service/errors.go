...
// FunctionLimitExceeded builds a 403 service Error for the
// max_functions_per_user / max_functions_per_member limits, enforced only
// at new-function creation (an existing function is never force-deleted
// just because a limit was later lowered below its owner's current
// count). The message states current/limit.
