...
// internalAPICallTimeout bounds each individual internalAPI call's
// underlying Go work (an internal/api handler invocation, which may touch
// the store). AsyncExport's fn (enginepool's internal.go) runs on its own
// goroutine with no access to the originating HTTP request's context --
// see this file's doc comment on internalAPIExport -- so this package must
// invent its own bound rather than inherit one.
const internalAPICallTimeout = 15 * time.Second

// internalAPIExport builds the "funcbox:internal" module's internalAPI
// enginepool.InternalExport: the guest-visible async function (method, path,
// bodyJSON, callerToken) returns a Promise (enginepool.AsyncExport's
...
// tokenKey on every single call (verifyCallerToken), because an
// InternalExport is built ONCE per pooled instance at warm-up
// (enginepool.Config.Internal) and then reused across every request that
// instance ever serves -- there is no way to rebuild it per request the way
// a per-request identity would normally suggest. The identity therefore has
// to travel as a per-CALL argument instead, carried by the guest from the
// X-Funcbox-Caller-Token request header server.go injects, and re-verified
// here every time. This is the "individual呼び出し引数として、signed token
// を検証する" design
//
...
// above.
func internalAPIExport(apiHandler *api.Handler, tokenKey []byte) enginepool.InternalExport {
	fn := func(cfg spidermonkey.Config, args []spidermonkey.Value) (spidermonkey.Value, error) {
		if len(args) < 4 {
			return nil, fmt.Errorf("internalAPI: expected 4 arguments (method, path, body, callerToken), got %d", len(args))
		}
		...
		if err != nil {
			...
			// AsyncExport's doc comment: a returned error rejects.
			return nil, fmt.Errorf("internalAPI: %w", err)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("internalAPI: marshal response: %w", err)
		}
		return spidermonkey.ValueOf(string(out)), nil
	}
	return enginepool.AsyncExport("internalAPI", fn)
}
