...
func (r *workspaceRepo) ByID(ctx context.Context, id string) (*store.Workspace, error) {
	return scanWorkspace(r.db.QueryRowContext(ctx,
...
func (r *workspaceRepo) Delete(ctx context.Context, id string) error {
	if _, err := r.db.ExecContext(ctx, `DELETE FROM workspace_members WHERE workspace_id = ?`, id); err != nil {
		return mapErr(err)
	}
	if _, err := r.db.ExecContext(ctx, `DELETE FROM workspaces WHERE id = ?`, id); err != nil {
		return mapErr(err)
	}
	return nil
}
